    ...
    function QuotaChip({ snapshot }) {
        ...
        const load = async () => {
          try {
            ...
            if (result && result.ok) {
              ...
              setFailed(true);
              const detail = result && result.error ? String(result.error) : "snapshot unavailable";
              setErrorText(detail.slice(0, 140));
            }
          } catch (error) {
            if (alive) {
              setFailed(true);
              setErrorText(String((error && error.message) || error).slice(0, 140));
            }
          }
        };
        load();
      ...
      const tip = limits
        .map((e) => {
          ...
          const calls =
            e.type === "TIME_LIMIT" && (e.used !== null || e.cap !== null)
              ? " · " + (e.used ?? "?") + "/" + (e.cap !== null ? (e.cap >= 1000 ? Math.round(e.cap / 100) / 10 + "k" : e.cap) : "?") + " calls"
              : "";
          const reset = e.resetTime ? " · resets in " + (resetsIn(e.resetTime) || "?") : "";
          return e.label + " " + pct + calls + reset;
        })
        .join("\n")
      ...
      return React.createElement(
        "a",
        {
          href: CONSOLE_URL,
          onClick: openConsole,
          "data-testid": "glm-quota-chip",
          "aria-label": "GLM Coding Plan quota",
